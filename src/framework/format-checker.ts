...
export class FormatChecker extends Base {
    ...
    async _messageHandler(message: Message): Promise<void> {
        ...
        if (this.checker instanceof RegExp) {
            ...
            this.checker instanceof Function &&
            this.checker(message.cleanContent, logger)
        )
            return;
        ...
        const warningContent = [
            'Bonjour,',
            `Le message que vous avez posté dans ${message.channel.toString()} est incorrectement formaté, il a donc été supprimé.`,
        ...
        try {
            await author
                ...
                .then(async (warning) => warning.suppressEmbeds(true));
        } catch (error: unknown) {
            logger.error(
                error,
            ...
            if (channel === undefined) {
                logger.fatal('text channel not found: logs');
                return;
            }
