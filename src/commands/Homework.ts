...
async function addEmbedFields(
    ...
    homeworks
        ...
        .map((homework, index) => {
            ...
            let value = `Due for ${DateTime.fromISO(
                homework.date,
            ).toLocaleString({
                locale: 'fr',
            })}`;
...
export default new Command({
    ...
    async handle({ message }) {
        ...
        if (
            ...
            message.reply(
                'You need to be a HEIG-VD student to use this command.',
            );
            return;
        }
        ...
        if (args[1] === 'add' && args.length === 4) {
            ...
            const ttl =
                Math.floor(
                    Math.abs(
                        Interval.fromDateTimes(DateTime.local(), date).length(
                            'seconds',
                        ),
                    ),
                ) + 86400; // + one day
        ...
        if (args[1] === 'modify' && args.length >= 4 && args.length <= 5) {
            ...
            if (keys.length) {
                ...
                const homework: Homework = JSON.parse(
